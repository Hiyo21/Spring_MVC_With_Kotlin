...
public class UserDaoJdbc extends UserDao {
	...
	public void add(User user){
		jdbcTemplate.update(this.sqlService.getSql("userAdd"), user.getId(), user.getName(), user.getPassword(), user.getEmail(), user.getLevel().value, user.getLogin(), user.getLikes());
	}
	...
	public int update(User toBeUpdated) {
		return jdbcTemplate.update(sqlService.getSql("userUpdate"), toBeUpdated.getName(), toBeUpdated.getPassword(), toBeUpdated.getLevel().value, toBeUpdated.getLogin(), toBeUpdated.getLikes(), toBeUpdated.getEmail(), toBeUpdated.getId());
	}
